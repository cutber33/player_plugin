...
export class Player {
  ...
  constructor(configuration) {
    ...
    this.layer.refresh(this.shown, null);
  ...
  onChange(newTimestamp, previousTimestamp) {
    this.shown = newTimestamp;
    if (this.layer) {
      this.layer.refresh(newTimestamp, previousTimestamp);
    }
    this.setTimestamp();
  }
  ...
  onForward() {
    if (this.shown == this.last) {
      ...
      this.onChangeCallback(nextTimestamp, this.shown);
    } else {
  ...
  onPickedTimestamp(click) {
    ...
    this.onChangeCallback(newTimestamp, this.shown);
  }
